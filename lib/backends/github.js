...
module.exports = class GitHubBackend {
  ...
  getIntegrations () {
    return this.github.pulls.list({
      ...
      const commitSHA = _.get(results, [ 'data', '0', 'head', 'sha' ])

      if (!commitSHA) {
        return null
      }

      return this.github.repos.listStatusesForRef({
        owner: this.owner,
        repo: this.repo,
        ref: commitSHA
      }).then((statusResults) => {
        const checks = _.chain(statusResults)
          .get([ 'data' ], [])
          .map('context')
          .map((check) => {
            if (_.startsWith(check, 'continuous-integration')) {
              return _.split(check, '/')[1]
            }
            return check
          })
          .uniq()
