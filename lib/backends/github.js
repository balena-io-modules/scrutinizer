  ...
  async readDirectoryFilePaths(directory) {
    try {
      ...
      if (_.isArray(results.data)) {
        ...
        const dirs = _.map(_.filter(results.data, { type: 'dir' }), 'path');

        const nestedFiles = await Bluebird.map(dirs, async dir => {
          const directoryFiles = await this.readDirectoryFilePaths(dir);
          const markdownFiles = _.filter(directoryFiles, file =>
            _.endsWith(file, 'md')
          );
          return markdownFiles;
        });

        return [...files, ..._.flatten(nestedFiles)];
      }
